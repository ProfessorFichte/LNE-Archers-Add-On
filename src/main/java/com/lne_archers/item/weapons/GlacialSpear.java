...
public class GlacialSpear extends SwordItem implements ConfigurableAttributes {
    ...
    public boolean postHit(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        if (FabricLoader.getInstance().isModLoaded("loot_n_explore")) {
            WeaponPassives.GlacialMelee(stack, target, attacker);
        }
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        if (FabricLoader.getInstance().isModLoaded("loot_n_explore")) {
            WeaponTooltips.GlacialMelee(stack, world, tooltip, context);
        }
